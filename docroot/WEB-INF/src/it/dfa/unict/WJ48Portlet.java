...
public class WJ48Portlet extends MVCPortlet {
	...
	private final Log _log = LogFactoryUtil.getLog(WJ48Portlet.class);
	...
	public void submit(ActionRequest actionRequest,
		...
		try {
			...
			if (uploadedFile != null && uploadedFile.length() == 0) {
				_log.warn("Uploaded an empity file.");
				...
				if (appPrefs.getApplicationId() <= 0) {
					...
					// Just a description of the job, could passed from the JSP
					// maybe.
					appInput.setDescription("WEKA J48 Application");

					String[] inputSandbox = null;
					File inputARFF = new File(filePath);
					...
					inputSandbox = new String[] { filePath };
					...
					try {
						...
						if (t.getStatus().equals("WAITING")
							...
							String t2 = client.uploadFile(uploadPath,
									inputSandbox);
							_log.info(t2);
							// TODO Check the FG response to see if the file was
							// correctly uploaded --> "gestatus": "triggered" in
							// the
							// respose notify user that task was correctly
							// submitted and
	...
	private File processInputFile(UploadPortletRequest uploadRequest,
		...
		File file = null;
		String fileInputName = "uploadDataset";
